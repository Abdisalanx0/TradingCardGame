...
export const InventoryProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchInventoryItems = async () => {
      try {
        ...
        const response = await fetch('http://localhost/php/userInventory.php', {
          method: 'POST',
          headers,
        ...
        if (response.ok) {
          ...
          console.log("Failed to fetch inventory items. HTTP status: ", response.status);
        }
      } catch (err) {
  ...
  return (
    <InventoryContext.Provider value={{ inventoryItems, setInventoryItems, inventoryItemsSort, setInventoryItemsSort, inventoryItemNameFilter, setInventoryItemNameFilter }}>
      {children}
